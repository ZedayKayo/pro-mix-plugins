// ═══════════════════════════════════════════════════════
// PRO-MIX PLUGINS — Login Page
// ═══════════════════════════════════════════════════════

import { createAuthLayout } from './AuthLayout.js';
import { loginUser, loginWithGoogleUser, isLoggedIn } from '../../core/store.js';
...
export function renderLoginPage() {
  ...
  const content = `
    ...
    <!-- Divider -->
    <div class="auth-divider stagger-up" style="animation-delay: 0.5s;"><span>or use direct access</span></div>
    ...
    <form class="auth-form" id="login-form">
      ...
      <div class="form-group stagger-up" style="animation-delay: 0.7s;">
        ...
        <div class="input-wrapper">
          ...
          <input type="${showPassword ? 'text' : 'password'}" class="input" id="auth-password" placeholder="••••••••" required autocomplete="current-password" />
      ...
      <!-- Options Row -->
      <div class="auth-options stagger-up" style="animation-delay: 0.8s;">
      ...
      <button type="submit" class="btn btn-primary btn-auth-submit stagger-up" style="animation-delay: 0.9s;" id="login-submit-btn">
        Initialize Session
      </button>
    </form>
    
    <div class="auth-footer stagger-up" style="animation-delay: 1.0s;">
      Don't have access? <a href="/register" class="auth-link">Join The Studio</a>
    </div>
  `;

  function render() {
    container.innerHTML = createAuthLayout(content, 'login');
    attachEvents();
  }
